...
class ImgChannelRMSNorm(nn.Module):
    def __init__(self, channel_size: int, eps: float = 1e-3):
        super().__init__()
        self.norm = nn.RMSNorm(channel_size, eps=eps)

    def forward(self, x: Tensor) -> Tensor:
        x = x.permute(0, 2, 3, 1)  # (B, C, H, W) -> (B, H, W, C)
        x = self.norm(x)  # apply on C
...
class Args:
    ...
    num_envs: int = 1
    action_repeat: int = 2  # y
    seed: int = 0
    device: str = "cuda"

    model_lr: float = 1e-4  # y
    model_eps: float = 1e-8  # y
    model_grad_clip: float = 1000.0  # y

    actor_lr: float = 3e-5  # y
    actor_eps: float = 1e-5  # y
    actor_grad_clip: float = 100.0  # y

    critic_lr: float = 3e-5  # y
    critic_eps: float = 1e-5  # y
    critic_grad_clip: float = 100.0  # y
...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.encoder = nn.Sequential(
            ...
            ImgChannelRMSNorm(32),
            nn.SiLU(),
            nn.Conv2d(32, 64, kernel_size=4, stride=2, padding=1, bias=False),
            ImgChannelRMSNorm(64),
            nn.SiLU(),
            nn.Conv2d(64, 128, kernel_size=4, stride=2, padding=1, bias=False),
            ImgChannelRMSNorm(128),
            nn.SiLU(),
            nn.Conv2d(128, 256, kernel_size=4, stride=2, padding=1, bias=False),
            ImgChannelRMSNorm(256),
            nn.SiLU(),
            nn.Flatten(),
...
class Decoder(nn.Module):
    def __init__(self):
        ...
        layers[-1].apply(uniform_weight_init(1.0))
...
class RewardPredictor(nn.Module):
    def __init__(self):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(args.stochastic_size + args.deterministic_size, 512),
            nn.SiLU(),
            nn.Linear(512, 512),
            nn.SiLU(),
            nn.Linear(512, 1),
        )
        [m.apply(weight_init) for m in self.net[:-1]]
        self.net[-1].apply(uniform_weight_init(0.0))
...
class Actor(nn.Module):
    def __init__(self, envs):
        ...
        [m.apply(weight_init) for m in self.actor[:-1]]
        self.actor[-1].apply(uniform_weight_init(1.0))
...
class Critic(nn.Module):
    def __init__(self):
        ...
        [m.apply(weight_init) for m in self.critic[:-1]]
        self.critic[-1].apply(uniform_weight_init(0.0))
...
envs = gym.vector.SyncVectorEnv([
    lambda: DMCWrapper(args.env_id.removeprefix("dm_control/"), args.seed, action_repeat=args.action_repeat)
    ...
    for epi in range(num_episodes):
        ...
        while True:
            ...
            if done.all():
                break
        cnt_episode += 1
        log.info(f"Episode {cnt_episode}, Return: {reward_sum.mean().item()}")
...
def world_model_learning(data: dict[str, Tensor]) -> tuple[Tensor, Tensor]:
    ...
    reconstructed_obs_loss = F.mse_loss(reconstructed_obs, data["observation"][:, 1:]).mean() * 100
    ...
    model_loss.backward()
    nn.utils.clip_grad_norm_(model_params, args.model_grad_clip)
    model_optimizer.step()

    writer.add_scalar("train/reconstruction_loss", reconstructed_obs_loss.item(), cnt_episode)
    writer.add_scalar("train/reward_loss", reward_loss.item(), cnt_episode)
    writer.add_scalar("train/kl_loss", kl_loss.item(), cnt_episode)
    writer.add_scalar("train/model_loss", model_loss.item(), cnt_episode)
...
def behavior_learning(posteriers_: Tensor, deterministics_: Tensor):
    ...
    actor_loss = -advantages.mean()  # direcly compute the gradient since the "dreamed environment" is differentiable
    actor_optimizer.zero_grad()
    actor_loss.backward()
    nn.utils.clip_grad_norm_(actor.parameters(), args.actor_grad_clip)
    ...
    value_loss.backward()
    nn.utils.clip_grad_norm_(critic.parameters(), args.critic_grad_clip)
    critic_optimizer.step()

    writer.add_scalar("train/actor_loss", actor_loss.item(), cnt_episode)
    writer.add_scalar("train/value_loss", value_loss.item(), cnt_episode)
...
def main():
    ...
    for i in range(args.num_iterations):
        ...
        toc = time.time()
        log.info(f"Time taken for training iteration {i}: {toc - tic:.2f} seconds")
